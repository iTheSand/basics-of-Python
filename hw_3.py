# 8_3. Создайте собственный класс-исключение, который должен проверять содержимое списка на наличие только чисел.
...
con = False
while con == False:
    inp_data = input("программа закончит работу при наборе спецсимвола # \nвведите число: ")
    if inp_data.isdigit() == True:
        li.append(inp_data)
    elif inp_data == "#":
        con = True
        print(f"результат - {li}")
    try:
        if inp_data.isdigit() == False:
            raise OwnError("Ошибка! Введите число! \n")
